...
describe("ton stake tests", () => {
  ...
  it("not admin try to update utonic manager", async () => {
    ...
    let userUpdate1 = await strategyTonContract.sendAdminUpdateUtonicManager(
  ...
  it("admin update code", async () => {
    ...
    let adminUpdate = await strategyTonContract.sendAdminUpdateUtonicManager(
    ...
    expect(strategyTonData.tonReceiverAddress.toString()).toBe(tonReceiver.address.toString())
    expect(strategyTonData.totalShares).toBe(0n)
    expect(strategyTonData.utonicManagerAddress.toString()).toBe(newUtonicManager.address.toString())
    expect(strategyTonData.withdrawPendingTime).toBe(BigInt(withdrawPendingTime))
