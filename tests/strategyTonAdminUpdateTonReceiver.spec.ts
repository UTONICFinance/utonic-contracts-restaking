...
import { STRATEGY_OP_ADMIN_DELEGATE_ACK, STRATEGY_OP_ADMIN_EXTRACT_TOKEN, STRATEGY_OP_ADMIN_UNDELEGATE_ACK, STRATEGY_OP_ADMIN_UPDATE_OPERATOR_SHARE, STRATEGY_OP_ADMIN_UPDATE_TOKEN_RECEIVER, STRATEGY_OP_ADMIN_UPDATE_UTONIC_MANAGER } from "../wrappers/stake/strategy/strategyOp";
...
describe("ton stake tests", () => {
  ...
  let newTonReceiver: SandboxContract<TreasuryContract>;
  ...
  beforeEach(async () =>  {
    ...
    newTonReceiver = await blockchain.treasury("newTonReceiver");
  ...
  it("not admin try to update utonic manager", async () => {
    ...
    let userUpdate1 = await strategyTonContract.sendAdminUpdateTokenReceiver(
    ...
    expect(userUpdate1.transactions).toHaveTransaction({
        ...
        op: STRATEGY_OP_ADMIN_UPDATE_TOKEN_RECEIVER
    })
    // nothing changed
  ...
  it("admin update code", async () => {
    ...
    let adminUpdate = await strategyTonContract.sendAdminUpdateTokenReceiver(
        admin.getSender(), 1, newTonReceiver.address, "0.1"
    )
    let strategyTonData = await strategyTonContract.getStrategyData()
    expect(strategyTonData.adminAddress.toString()).toBe(admin.address.toString())
    expect(strategyTonData.pendingAdminAddress.toString()).toBe(admin.address.toString())
    expect(strategyTonData.debtToken).toBe(0n)
    expect(strategyTonData.strategyId).toBe(2n)
    expect(strategyTonData.tonReceiverAddress.toString()).toBe(newTonReceiver.address.toString())
    expect(strategyTonData.totalShares).toBe(0n)
    expect(strategyTonData.utonicManagerAddress.toString()).toBe(utonicManagerContract.address.toString())
    expect(strategyTonData.withdrawPendingTime).toBe(BigInt(withdrawPendingTime))
