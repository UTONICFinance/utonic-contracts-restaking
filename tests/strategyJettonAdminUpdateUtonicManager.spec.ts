...
import { STRATEGY_OP_ADMIN_DELEGATE_ACK, STRATEGY_OP_ADMIN_EXTRACT_TOKEN, STRATEGY_OP_ADMIN_UNDELEGATE_ACK, STRATEGY_OP_ADMIN_UPDATE_OPERATOR_SHARE, STRATEGY_OP_ADMIN_UPDATE_UTONIC_MANAGER } from "../wrappers/stake/strategy/strategyOp";
...
describe("ton stake tests", () => {
  ...
  let newUtonicManager: SandboxContract<TreasuryContract>;
  let fakeUtonicManager: SandboxContract<TreasuryContract>;
  ...
  beforeEach(async () =>  {
    ...
    newUtonicManager = await blockchain.treasury("newUtonicManager");
    fakeUtonicManager = await blockchain.treasury("fakeUtonicManager");
  ...
  it("not admin try to update utonic manager", async () => {
    ...
    let userUpdate1 = await strategyJettonContract.sendAdminUpdateUtonicManager(
        user1.getSender(), 1, fakeUtonicManager.address, "0.1"
    )
    expect(userUpdate1.transactions).toHaveTransaction({
        to: strategyJettonContract.address,
        exitCode: STAKE_ERR_UNAUTHORIZED,
        op: STRATEGY_OP_ADMIN_UPDATE_UTONIC_MANAGER
    })
    // nothing changed
  ...
  it("admin update code", async () => {
    ...
    let adminUpdate = await strategyJettonContract.sendAdminUpdateUtonicManager(
        admin.getSender(), 1, newUtonicManager.address, "0.1"
    )
    let strategyJettonData = await strategyJettonContract.getStrategyData()
    expect(strategyJettonData.adminAddress.toString()).toBe(admin.address.toString())
    expect(strategyJettonData.pendingAdminAddress.toString()).toBe(admin.address.toString())
    ...
    expect(strategyJettonData.strategyId).toBe(2n)
    expect(strategyJettonData.totalShares).toBe(0n)
    expect(strategyJettonData.utonicManagerAddress.toString()).toBe(newUtonicManager.address.toString())
    expect(strategyJettonData.withdrawPendingTime).toBe(BigInt(withdrawPendingTime))
