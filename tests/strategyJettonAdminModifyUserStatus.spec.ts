...
describe("ton stake tests", () => {
  ...
  it("cancel pending delegate", async () => {
    ...
    let userCancel2 = await strategyJettonContract.sendAdminCancelUserPending(
    ...
    let adminCancel = await strategyJettonContract.sendAdminCancelUserPending(
  ...
  it("admin delegate ack", async () => {
    ...
    let userDelegateAck2 = await strategyJettonContract.sendAdminDelegateAck(
    ...
    let adminUndelegateAck = await strategyJettonContract.sendAdminUndelegateAck(
    ...
    let adminDelegateAck = await strategyJettonContract.sendAdminDelegateAck(
        admin.getSender(), 1, userInfoContract1.address, userResponse2.address, "0.1"
    )
    userInfoData1 = await userInfoContract1.getUserStrategyInfoData()
    expect(userInfoData1.withdrawCnt).toBe(0n);
  ...
  it("admin undelegate ack", async () => {
    ...
    let userUndelegateAck2 = await strategyJettonContract.sendAdminUndelegateAck(
        user2.getSender(), 1, userInfoContract1.address, userResponse2.address, "0.1"
    )
