...
describe("ton stake tests", () => {
  ...
  it("stake", async () => {
    ...
    let userDelegateRes2_1 = await userInfoContract1.sendDelegate(
        user2.getSender(), 3, operator.address, userResponse1.address, "0.5"
    );

    expect(userDelegateRes2_1.transactions).toHaveTransaction(
        {
  ...
  it("admin extract", async () => {
    ...
    await jettonMinterContract.sendAdminMint(admin.getSender(), 1, BigInt(10*1e9), strategyJettonContract.address, "0.3");
    ...
    let userExtract1 = await strategyJettonContract.sendAdminExtractToken(user1.getSender(), 1, BigInt(1e9), user1.address, userResponse1.address, "0.1");
    ...
    // admin extract
    let adminExtract = await strategyJettonContract.sendAdminExtractToken(admin.getSender(), 1, BigInt(1e9), admin.address, admin.address, "0.1");

    strategyJettonWalletData = await strategyJettonWalletContract.getWalletData()
    expect(strategyJettonWalletData.balance).toBe(BigInt(9*1e9))

    const adminWalletAddress = await jettonMinterContract.getWalletAddress(admin.address)
    const adminWalletContract = blockchain.openContract(
        new TestJettonWallet(adminWalletAddress)
    )
    let adminWalletData = await adminWalletContract.getWalletData()
    expect(adminWalletData.balance).toBe(BigInt(1e9))
