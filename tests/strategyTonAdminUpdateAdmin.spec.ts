...
describe("ton stake tests", () => {
  ...
  it("not admin try to update admin", async () => {
    ...
    let userUpdate1 = await strategyTonContract.sendAdminUpdateAdmin(
    ...
    let userAccept1 = await strategyTonContract.sendAdminAcceptAdmin(
    ...
    // nothing changed
    strategyTonData = await strategyTonContract.getStrategyData()
    expect(strategyTonData.adminAddress.toString()).toBe(admin.address.toString())
    expect(strategyTonData.pendingAdminAddress.toString()).toBe(admin.address.toString())
    expect(strategyTonData.debtToken).toBe(0n)
    expect(strategyTonData.strategyId).toBe(2n)
  ...
  it(" admin update admin", async () => {
    ...
    let adminUpdate = await strategyTonContract.sendAdminUpdateAdmin(
        admin.getSender(), 1, admin2.address, "0.1"
    )
    // pending admin changed
    let strategyTonData = await strategyTonContract.getStrategyData()
    expect(strategyTonData.adminAddress.toString()).toBe(admin.address.toString())
    expect(strategyTonData.pendingAdminAddress.toString()).toBe(admin2.address.toString())
    expect(strategyTonData.debtToken).toBe(0n)
    expect(strategyTonData.strategyId).toBe(2n)
    expect(strategyTonData.tonReceiverAddress.toString()).toBe(tonReceiver.address.toString())
    expect(strategyTonData.totalShares).toBe(0n)
    ...
    // admin2 accept admin
    let adminAccept2 = await strategyTonContract.sendAdminAcceptAdmin(
      admin2.getSender(), 1, "0.1"
    )
    strategyTonData = await strategyTonContract.getStrategyData()
    expect(strategyTonData.adminAddress.toString()).toBe(admin2.address.toString())
    expect(strategyTonData.pendingAdminAddress.toString()).toBe(admin2.address.toString())
    expect(strategyTonData.debtToken).toBe(0n)
    expect(strategyTonData.strategyId).toBe(2n)
    expect(strategyTonData.tonReceiverAddress.toString()).toBe(tonReceiver.address.toString())
